...
namespace ITP4915
{
	public partial class ModifyDelivery : Form
	{
		...
		string search;
		public ModifyDelivery()
		{
			InitializeComponent();
		}

		private void ModifyDelivery_Load(object sender, EventArgs e)
		{
			...
			sqlCmd.CommandText = "SELECT * FROM ITP4915.delivery_emp";
			sqlCmd.CommandType = CommandType.Text;
			sqlRd = sqlCmd.ExecuteReader();

			while (sqlRd.Read())
			{
				string empid = (sqlRd["delivery_empID"].ToString());
		...
		private void buttonUpdate_Click(object sender, EventArgs e)
		{
			...
			MySqlCommand sqlCmd = new MySqlCommand();
			sqlCmd.Connection = sqlConn;
			sqlCmd.CommandText = "UPDATE Delivery,customer,delivery_empGeneral SET  Delivery.DeliveryDate =@DeliveryDate,customer.customerName=@Name,customer.address=@address,customer.PhoneNum=@PhoneNum,delivery_empGeneral.delivery_empID=@empID  WHERE Delivery.customerID = customer.customerID AND Delivery.DeliveryID =delivery_empGeneral.deliveryID AND Delivery.DeliveryID=@DeliveryID";
